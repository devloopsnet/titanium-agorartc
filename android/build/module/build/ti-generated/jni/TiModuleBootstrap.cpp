...
#define TAG "net.devloops.AgoraRTC"
...
static void TiModule_getBinding(const FunctionCallbackInfo<Value>& args)
{
	...
	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "net.devloops.AgoraRTC TiModule_getBinding() requires 1 argument: binding");
	...
	if (maybeBinding.IsEmpty()) {
		titanium::JSException::Error(isolate, "net.devloops.AgoraRTC TiModule_getBinding requires 1 argument: binding. Received argument that could not be converted to a String");
		args.GetReturnValue().Set(scope.Escape(Undefined(isolate)));
		return;
	}
...
Java_net_devloops_AgoraRTC_TiModuleBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("net.devloops.AgoraRTC", &TiModuleBinding);
