...
class NetDevloopsAgoraRTCModule: TiModule, AgoraRtcEngineDelegate {
    ...
    @objc(enableVideo:)
    func enableVideo(_ a: Array<Any>?)  { NetDevloopsAgoraRTCModule.sharedEngine?.enableVideo() }
    
    @objc(disableVideo:)
    func disableVideo(_ a: Array<Any>?) { NetDevloopsAgoraRTCModule.sharedEngine?.disableVideo() }
    
    @objc(startPreview:)
    func startPreview(_ a: Array<Any>?) { NetDevloopsAgoraRTCModule.sharedEngine?.startPreview() }
    
    @objc(stopPreview:)
    func stopPreview(_ a: Array<Any>?)  { NetDevloopsAgoraRTCModule.sharedEngine?.stopPreview() }
    
    @objc(muteLocalVideoStream:)
    func muteLocalVideoStream(_ a: Array<Any>?)      { NetDevloopsAgoraRTCModule.sharedEngine?.muteLocalVideoStream(boolArg(a, 0)) }
    
    @objc(muteAllRemoteVideoStreams:)
    func muteAllRemoteVideoStreams(_ a: Array<Any>?) { NetDevloopsAgoraRTCModule.sharedEngine?.muteAllRemoteVideoStreams(boolArg(a, 0)) }
    
    @objc(muteRemoteVideoStream:)
    func muteRemoteVideoStream(_ a: Array<Any>?)     { NetDevloopsAgoraRTCModule.sharedEngine?.muteRemoteVideoStream(UInt(intArg(a, 0)), mute: boolArg(a, 1)) }
    
    @objc(switchCamera:)
    func switchCamera(_ a: Array<Any>?)              { NetDevloopsAgoraRTCModule.sharedEngine?.switchCamera() }
    
    @objc(enableDualStreamMode:)
    func enableDualStreamMode(_ a: Array<Any>?)      { NetDevloopsAgoraRTCModule.sharedEngine?.enableDualStreamMode(boolArg(a, 0)) }
    ...
    func setVideoEncoderConfiguration(_ arguments: Array<Any>?) {
        ...
        let frameRate = (p["frameRate"] as? NSNumber)?.intValue ?? 15
